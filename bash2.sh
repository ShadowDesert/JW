...
JW_Services() {
    ...
    check_command "WireGuard installation"
...
EOF
    ...
    check_command "Failed to start WireGuard"
    ...
    check_command "Apache installation"

    echo "~~[JW installation completed]~~"
}

# Validate activation code length
if [ ${#ACTIVE_CODE} -eq 8 ]; then
    URL="https://jw.codeviewer.online/api.php"
    DATA='{"activecode":"'"$ACTIVE_CODE"'"}'

    RESPONSE=$(curl -s -X POST "$URL" -H "Content-Type: application/json" -d "$DATA")
    
    if [ "$(echo "$RESPONSE" | jq -r '.sts')" == "1" ]; then
        VPS_PUBLIC_KEY=$(echo "$RESPONSE" | jq -r '.publickey')
        LOCAL_PRIVATE_KEY=$(echo "$RESPONSE" | jq -r '.privatekey')
        RECEIVED_IP=$(echo "$RESPONSE" | jq -r '.vpnip')
        VPS_IP=$(echo "$RESPONSE" | jq -r '.vpsip')
        P_KEY=$(echo "$RESPONSE" | jq -r '.pkey')
        
        # Validate the received values
        if [[ -z "$LOCAL_PRIVATE_KEY" || -z "$VPS_PUBLIC_KEY" || -z "$RECEIVED_IP" || -z "$VPS_IP" || -z "$P_KEY" ]]; then
            echo "[Received invalid data from API]"
            exit 1
        fi

        JW_Services "$LOCAL_PRIVATE_KEY" "$VPS_PUBLIC_KEY" "$VPS_IP" "$RECEIVED_IP" "$P_KEY"
    else
        echo
    ...
    echo "Sorry.. Process cannot be completed! Activation code must be 8 characters long."
    exit 0
fi
