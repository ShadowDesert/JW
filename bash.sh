...
if [ ${#ACTIVE_CODE} -eq 5 ]; then
    URL="https://api.example.com/endpoint"
    DATA='{"activecode":"'"$ACTIVE_CODE"'"}'

    RESPONSE=$(curl -s -X POST "$URL" -H "Content-Type: application/json" -d "$DATA")
    
    if [ "$(echo "$RESPONSE" | jq -r '.sts')" == "good" ]; then
        ...
        if [[ -z "$LOCAL_PRIVATE_KEY" || -z "$VPS_PUBLIC_KEY" || -z "$RECEIVED_IP" || -z "$VPS_IP" ]]; then
            echo "[Received invalid data from API]"
            exit 1
        fi

        JW_Services "$LOCAL_PRIVATE_KEY" "$VPS_PUBLIC_KEY" "$VPS_IP" "$RECEIVED_IP"
    else
        echo
